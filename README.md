...
|---|---|
| [Document Fabric: Drive to Qdrant](workflows/document-fabric-drive-to-qdrant/) | The ingestion half of a high-accuracy RAG pipeline. Watches a Google Drive folder and turns any document (PDF, Word, PowerPoint, Excel, CSV, HTML, text, images, and native Google files) into structure-aware, metadata-tagged vectors in Qdrant. Converts to Markdown via a bundled MarkItDown service, chunks on the heading hierarchy (with heuristic recovery for flat PDFs), tags rich filterable metadata, and upserts idempotently. Generic across documentation types: an AI setup prompt tailors the metadata fields to your docs. |
